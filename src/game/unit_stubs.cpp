...
inline void LIST_INT::Expand(int p_max)
{
	if (p_max > m_max) {
		int* old = m_data;
...
inline void LIST_INT::Read(STREAM* p_stream)
{
	p_stream->Read(&m_n, 4);
	Expand(m_n);
...
inline void LIST_SHORT::Expand(int p_max)
{
	if (p_max > m_max) {
		short* old = m_data;
...
inline void LIST_SHORT::Read(STREAM* p_stream)
{
	p_stream->Read(&m_n, 4);
	Expand(m_n);
	...
	: TERRAIN(p_vid, p_x, p_y, p_z, p_dir, p_parent)
{
	...
	m_unk0x8c = m_child && m_child->m_vid == m_vid->m_linkVid
...
static inline int UnitHaveArmedLink(UNIT* p_unit)
{
...
decomp_intptr UNIT::Action(int p_action, int p_a, int p_b, int p_c)
{
			...
			if (m_unk0x8c & 1) {
				...
				if (!cmd || cmd == 4 || cmd == 16) {
					if (m_endCadr > m_begCadr
						|| (UnitHaveArmedLink(this) ? m_child->m_unk0x50 : m_unk0x50)
						|| !(rand() % 11)) {
						SPRITE* enemy = SeekEnemy();
						if (enemy)
							SetCommand(4, enemy);
...
lastGuard:
	...
	case 85: { // ACT_DAMAGE
		...
		if (p_a >= 0 && 0 == (m_flag & 0x7c)
			&& (m_exData ? m_exData->m_unk0x20 : m_vid->m_unk0x2c) != 0.0f) {
			unsigned char dir = rand() % 256;
			float z = m_z;
			float y = m_y;
