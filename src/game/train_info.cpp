...
void TRAIN_INFO::AddEngine(const ENGINE* p_engine)
{
	...
	if (shots > 0) {
		...
		if (child && child->m_vid == p_engine->m_vid->m_linkVid && child->m_vid->m_weaponVid &&
			child->m_vid->m_weapon && p_engine->m_vid->m_exData->m_unk0x18 == 0.0f) {
	...
	if (shots) {
		if (p_engine->m_vid->m_idx == 82) {
			VID* shell = Map->m_noVid > 70 && Map->m_vids[70] ? Map->m_vids[70] : EmptyVid;
