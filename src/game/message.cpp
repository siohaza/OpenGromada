...
MESSAGE::MESSAGE(int p_messageVid, int p_markerVid, float p_z, float p_y, int p_n, unsigned long p_tactDelay)
	: m_n(p_n), m_tactDelay(p_tactDelay), m_messageVid(p_messageVid), m_markerVid(p_markerVid), m_z(p_z), m_y(p_y)
{
	m_lineSpacing = -1;
	int i = 0;
	if (i < m_n) {
...
void MESSAGE::DeletePointerToSprite(SPRITE* p_sprite)
{
	...
	if (m_n > 0) {
		do {
			if (m_data2[i] == p_sprite) {
				m_data2[i] = 0;
			}
			if (m_data[i] == p_sprite) {
				m_data[i] = 0;
			}
			++i;
		} while (i < m_n);
...
void MESSAGE::Shift()
{
	...
	int height = (int) UI_SCALING::ScaleMetric(
		(float) (vid->m_messageLineHeight + 1),
		((GRAPH_CORE*) Graph)->m_uiScale * ((GRAPH_CORE*) Graph)->m_uiPresentationScale
	);
	...
	if ((Map->m_menu.m_state & 1) && Map->m_menu.NVidUnderCursor() == m_markerVid) {
		int row = (int) (Map->m_menu.m_underCursor->m_y - Map->m_menu.m_underCursor->m_z - Map->m_shiftY -
						 Graph->m_viewYMin) /
				  height;
...
void MESSAGE::Put(const STRING& p_msg, float p_x, float p_y)
{
	...
	if (strcmp(p_msg.m_str, empty_str)) {
		...
		if (!m_data2[m_n - 1]) {
			m_data2[m_n - 1] = Map->CreateSprite(
				Map->Vid(m_messageVid),
				m_z,
	...
	if (i < m_n) {
		...
		do {
			...
			if (spr) {
				...
				p[-1] = *p;
				m_targetX[i - 1] = m_targetX[i];
				m_targetY[i - 1] = m_targetY[i];
				*p = 0;
			}
			++i;
			++p;
