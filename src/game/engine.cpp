...
inline static void EngineDebugIndices(
	int& p_best_result,
	int& p_goal_result,
	SPRITE* p_best,
	SPRITE* p_goal,
...
void ENGINE::DrawSecondaryInfo()
{
	...
	GRAPH_CORE::PrintfXY(
		core,
		(viewXMin = core->m_viewXMin) + 22.0f,
		y,
	...
	if (m_actions.m_n) {
		...
		for (int i = 0; i < m_actions.m_n; ++i) {
			...
			// The action arguments are pointer-wide: several opcodes carry a
			// sprite in them, so %i would print half of one.
			text += Printf(
				"%i(%lli,%lli,%lli) ",
	...
	DrawGoalLine();
	if (m_commandOwner) {
		Graph->PutBigPixel(m_commandOwner->ScreenX(), m_commandOwner->ScreenY(), GRAPH_CORE::RED);
	}
}
...
void ENGINE::DrawGoalLine()
{
	if (m_secondaryCommandDot) {
		Graph->Line(
			...
			m_secondaryCommandDot->GetScreenY() + 2.0f,
			GRAPH_CORE::WHITE
		);
	}
	if (m_commandPathDot) {
		Graph->Line(
			...
			m_commandPathDot->GetScreenY() + 2.0f,
			GRAPH_CORE::WHITE
		);
	}
	if (m_goal) {
		Graph->Line(ScreenX(), ScreenY(), Goal()->ScreenX(), Goal()->ScreenY(), GRAPH_CORE::GREEN);
	}
	...
	if (m_commandDot) {
		Graph->Line(ScreenX(), ScreenY(), m_commandDot->GetScreenX(), m_commandDot->GetScreenY(), GRAPH_CORE::BLUE);
	}
}

// STUB: ALIEN 0x44f320
decomp_intptr ENGINE::Action(int p_cmd, decomp_intptr p_a, decomp_intptr p_b, decomp_intptr p_c)
{
	switch (p_cmd & 0xff) {
	case 86: { // ACT_REPAIR
		VID* vid = m_vid;
		if (vid->m_idx != 82) {
			return PreserveDelegatedActionResult(UNIT::Action(p_cmd, p_a, p_b, p_c));
		}
		if (m_ammo / 64 >= ((vid->m_linkVid && vid->m_linkVid->m_aniChildVid[8] && vid->m_linkVid->m_weapon)
		...
		VID* shellVid = (Map->m_noVid > 70 && Map->m_vids[70]) ? Map->m_vids[70] : EmptyVid;
		...
		if (carried + shells < ((vid->m_linkVid && vid->m_linkVid->m_aniChildVid[8] && vid->m_linkVid->m_weapon)
									? vid->m_linkVid->m_exData->m_maxAmmo
									: vid->m_exData->m_maxAmmo) *
								   engines) {
			return PreserveDelegatedActionResult(UNIT::Action(p_cmd, p_a, p_b, p_c));
		}
		return PreserveDelegatedActionResult(TERRAIN::Action(p_cmd, p_a, p_b, p_c));
	}
	...
	case 90: // ACT_GET_GOAL
		return (decomp_intptr) FirstEngine()->m_goal;
	...
	case 70: // ACT_BACKUP_COMMAND
		m_actions.Insert(
			ACT((((m_flag >> 2) & 0x1f) << 8) + 74,
				(decomp_intptr) m_goal,
				(decomp_intptr) m_commandDot,
				(decomp_intptr) m_secondaryCommandDot)
		);
		if (m_goal) {
			++m_goal->m_noRef;
		}
	...
	case 130: { // ACT_NEXT_COMMAND
		...
		VID* vid = m_vid;
		int armed =
			(vid->m_aniChildVid[8] && vid->m_weapon) || (m_child && m_child->m_vid == vid->m_linkVid &&
														 m_child->m_vid->m_aniChildVid[8] && m_child->m_vid->m_weapon);
		if (armed) {
			ActNextCommandFighter();
		}
		if ((m_flag & 0x7c) != 0x60 || m_vid->m_idx != 85 || !m_unk0xec || m_speed != 0.0f) {
			return 0;
		}
		if (!m_unk0xf0) {
			...
			if ((int) (CurrentTime - m_unk0xf0) > Const->m_repairDockTime) {
				if (m_ammo / 64 > 0) {
					...
					SPRITE* crate = Map->CreateSprite(Map->Vid(86), GetX(), GetY(), GetZ(), Direction(), this);
					if (crate) {
	...
	case 85: { // ACT_DAMAGE
		if (p_a > 0) {
			...
			if (!(first->m_flag & 2) && p_b && ((m_flag ^ ((SPRITE*) p_b)->m_flag) & 0x1800)) {
				Map->ScriptRun(EvFunctionNumber[10], this, (SPRITE*) p_b, 0);
				first->m_flag |= 2;
			}
		}
		SPRITE* child = m_child;
		if (child && child->m_vid == m_vid->m_linkVid && child->m_vid->m_maxHp[(child->m_flag >> 11) & 3]) {
			return PreserveDelegatedActionResult(child->Action(p_cmd, p_a, p_b, p_c));
		}
		return PreserveDelegatedActionResult(SPRITE::Action(p_cmd, p_a, p_b, p_c));
	}
	case 80: { // ACT_SAVE
	...
	case 152: // ACT_FORCELINK_ENGINE
		if (!p_a || ((SPRITE*) p_a)->m_vid->m_sprClass != 21 || InTrain((SPRITE*) p_a)) {
			return 0;
		}
	...
	case 153: { // ACT_TRAIN_BEHAVE
		...
		for (; e; e = e->m_nextEngine) {
			...
			int maxAmmo = (link && link->m_aniChildVid[8] && link->m_weapon) ? link->m_exData->m_maxAmmo
																			 : vid->m_exData->m_maxAmmo;
	...
	case 154: // ACT_FIRST_ENGINE
		return (decomp_intptr) FirstEngine();
	case 155: // ACT_LAST_ENGINE
		return (decomp_intptr) LastEngine();
	case 156: // ACT_NEXT_ENGINE
		return (decomp_intptr) m_nextEngine;
...
void ENGINE::SetCommandToTrain(int p_cmd, SPRITE* p_goal, R_DOT* p_dot, R_DOT* p_secondaryDot)
{
	...
	if (p_cmd == 25) {
		if (!p_secondaryDot) {
			secondaryDot = RailMap.GetNearestDot_xyr(
				(int) ((SPRITE*) this)->X(),
				(int) ((SPRITE*) this)->Y(),
				(int) ((SPRITE*) this)->Z()
			);
	...
	for (engine = FirstEngine(); engine; engine = engine->m_nextEngine) {
		...
		if ((p_cmd == 28 || p_cmd == 29) && m_vid->m_idx != 45 && m_vid->m_exData->m_unk0x10 == 0.0f) {
...
void ENGINE::ActNextCommandFighter()
{
	...
	if (m_vid->m_idx != 35 || (HaveFightLink() && (linkedVid = m_child->m_vid)->m_weaponVid && linkedVid->m_weaponIdx &&
		...
		if (!(m_flag & 1)) {
			...
			if (cmd == 112 || cmd == 116) {
				...
				if (!owner || owner == this) {
					SPRITE* goal = m_goal;
					if (goal != m_child->m_goal && (goal->m_vid == EmptyVid || CanAttackThisSprite(m_goal))) {
						...
						if (child && child->m_vid == m_vid->m_unk0x5c && child->m_vid->m_weaponVid &&
							child->m_vid->m_weapon && m_vid->m_exData->m_unk0x18 == 0.0f) {
				...
				if (child) {
					...
					if (childVid == m_vid->m_unk0x5c && childVid->m_weaponVid && childVid->m_weapon) {
			...
			if (state == 1 || state == 2 || state == 5 || state == 6) {
				if ((m_unk0x8c & 1) && (m_child->m_unk0x50 || GameRand() % 4 == 0)) {
...
int ENGINE::NeedRepairByRepair()
{
	...
	if (hp < maxHp) {
		...
		if (linkVid && (!m_child || m_child->m_vid != linkVid)) {
			return 100 - 100 * hp / (maxHp + linkVid->m_maxHp[(m_flag >> 11) & 3]);
		}
	...
	if (child && child->m_vid == linkVid && child->m_vid->m_sprClass != 9) {
		...
		if (child->m_unk0x54 < child->m_vid->m_maxHp[childArmy]) {
			return 100 - 100 * (child->m_unk0x54 + hp) / (maxHp + child->m_vid->m_maxHp[childArmy]);
		}
	}
	return 0;
}
...
int ENGINE::TrainWeaponRange()
{
	...
	if (engine) {
		...
		for (engine = FirstEngine(); engine; engine = engine->m_nextEngine) {
			if (engine->m_child && engine->m_child->m_vid == engine->m_vid->m_unk0x5c &&
				engine->m_child->m_vid->m_weaponVid && engine->m_child->m_vid->m_unk0x40 && engine->m_ammo / 64 > 0) {
				if (engine->m_vid->m_idx == 35) {
					if (engine->m_vid->m_exData->m_unk0x18 < range) {
						range = engine->m_vid->m_exData->m_unk0x18;
					}
...
void ENGINE::ReCalcMoveParameters()
{
	...
	if (front->m_unk0xb0 == 0.0f && info.m_accelTime != 0.0f && (int) (info.m_speedInc / info.m_accelTime * 8.0f) > 7) {
		unsigned int flag = front->m_flag;
		if ((flag & 0x80) && !globaldeleting) {
			R_DOT* goal = front->m_commandDot;
			SPRITE* target = goal ? 0 : front->m_goal;
			int nostep = front->m_curDotRef.NoStepToTarget(goal, target, (flag >> 2) & 0x1f, front);
			MAN* flagman = Map->Flagman(Map->m_curArmy);
			if (front->InTrain(flagman) && !(Map->Flagman(Map->m_curArmy)->m_flag & 0x1800)) {
	...
	if (front->m_unk0xb0 < 0.0f) {
		...
		front->m_unk0xb0 = info.m_unk0x18 * 0.001f;
		int speed = (info.m_accelTime != 0.0f) ? (int) (info.m_speedInc / info.m_accelTime * 8.0f) : 0;
...
{
	float fx = (float) (((m_curDotRef.m_pos * (m_curDotRef.LinkedDot()->m_x - m_curDotRef.m_dot->m_x)) << 8) /
						m_curDotRef.LinkDist()) +
			   m_curDotRef.m_dot->m_x * 256.0f;
	float fy = (float) (((m_curDotRef.m_pos * (m_curDotRef.LinkedDot()->m_y - m_curDotRef.m_dot->m_y)) << 8) /
						m_curDotRef.LinkDist()) +
			   m_curDotRef.m_dot->m_y * 256.0f;
	float fz = (float) (((m_curDotRef.m_pos * (m_curDotRef.LinkedDot()->m_z - m_curDotRef.m_dot->m_z)) << 8) /
						m_curDotRef.LinkDist()) +
			   m_curDotRef.m_dot->m_z * 256.0f;
	...
	if ((rearLinked = m_lastDotRef.LinkedDot()) != 0) {
		float bx = (float) (((m_lastDotRef.m_pos * (rearLinked->m_x - m_lastDotRef.m_dot->m_x)) << 8) /
							m_lastDotRef.LinkDist()) +
				   m_lastDotRef.m_dot->m_x * 256.0f;
		float by = (float) (((m_lastDotRef.m_pos * (rearLinked->m_y - m_lastDotRef.m_dot->m_y)) << 8) /
							m_lastDotRef.LinkDist()) +
				   m_lastDotRef.m_dot->m_y * 256.0f;
		float bz = (float) (((m_lastDotRef.m_pos * (rearLinked->m_z - m_lastDotRef.m_dot->m_z)) << 8) /
							m_lastDotRef.LinkDist()) +
				   m_lastDotRef.m_dot->m_z * 256.0f;

		ChangeCoor((bx + fx) * 0.001953125f, (by + fy) * 0.001953125f, (bz + fz) * 0.001953125f);
...
inline static ANGLE DirOf(const R_DOT_REF* p_ref)
{
...
int ENGINE::ForceLink(ENGINE* p_other)
{
	...
	m_lastDotRef.m_dot = m_curDotRef.m_dot->m_links[m_curDotRef.m_dot->GetLink_dir(tailRef->GetAngle())].m_dot;
...
inline static float SinOf(unsigned char p_dir)
{
	return ANGLE::SinTable[p_dir];
}
inline static float CosOf(unsigned char p_dir)
{
	return ANGLE::CosTable[p_dir];
}

inline static void StepRefToNearerDot(R_DOT_REF* p_ref, const SPRITE* p_sprite, float p_z, float p_y, float p_x)
{
	...
	if (sqrt((double) (dx * dx + dy * dy + dz * dz)) > sqrt((double) (lx * lx + ly * ly + lz * lz))) {
...
void ENGINE::SetRDot()
{
	...
	for (i = dot->m_noLinks - 1; i >= 0; --i) {
		...
		if ((d1 < d2 ? d1 : d2) > 0x6c || (d2 = d1 < d2 ? d1 : d2) < 0x14) {
			break;
		}
	}
	if (i < 0) {
	...
	dot->SetNearestPos(
		(int) (SinOf(dir.m_dir) * halfLen + m_x),
		(int) (Y() - CosOf(dir.m_dir) * halfLen),
		(int) m_z,
		&m_curDotRef
	);
...
ENGINE* ENGINE::GetBadIntersecting()
{
	...
	if (m_curDotRef.LinkedDot()) {
		...
		if (m_curDotRef.LinkedDot()->m_links[m_curDotRef.m_dot->m_links[m_curDotRef.m_link].m_backLink].m_crossLink) {
			R_DOT_LINK* cross =
				m_curDotRef.LinkedDot()->m_links[m_curDotRef.m_dot->m_links[m_curDotRef.m_link].m_backLink].m_crossLink;
			ENGINE* e = cross->m_dot->m_busyEngine;
			if (e && !e->InTrain(this)) {
				return cross->m_dot->m_busyEngine;
			}
...
inline static void SetRef(R_DOT_REF* p_ref, R_DOT* p_dot, int p_dist, int p_back, int p_pos)
{
	p_ref->m_dot = p_dot;
...
inline static int RefPosOf(const R_DOT_REF* p_ref)
{
	return p_ref->m_pos;
}
...
void ENGINE::PullTail(R_DOT_REF* p_ref)
{
	...
	if ((float) frontPos > m_vid->m_exData->m_unk0x08) {
		SetRef(
			&m_lastDotRef,
			m_curDotRef.m_dot->m_links[m_curDotRef.m_link].m_dot,
			m_curDotRef.m_dot->m_links[m_curDotRef.m_link].m_dist,
			m_curDotRef.m_dot->m_links[m_curDotRef.m_link].m_backLink,
			m_curDotRef.m_pos
		);
		...
		if (refDot == m_curDotRef.m_dot) {
			...
			if (idx < 0) {
				...
				idx = m_curDotRef.m_dot->GetLink_dir(m_curDotRef.GetAngle().m_dir + 0x80);
			...
			if ((float) (m_curDotRef.m_pos + dist) > m_vid->m_exData->m_unk0x08) {
				if (m_curDotRef.m_link == idx) {
					m_lastDotRef.m_pos = (int) m_vid->m_exData->m_unk0x08 - m_curDotRef.m_pos + dist;
				}
				else {
				...
				m_lastDotRef.m_pos = (int) m_vid->m_exData->m_unk0x08 - dist - m_curDotRef.m_pos;
			}
		}
			...
			if ((float) (frontPos + (refDot ? refDot->m_links[p_ref->m_link].m_dist : 0)) >
				...
				m_lastDotRef.m_pos =
					(int) m_vid->m_exData->m_unk0x08 - frontPos - (refDot ? refDot->m_links[p_ref->m_link].m_dist : 0);
			}
		}
		else {
			if ((float) (frontPos + (refDot ? refDot->m_links[p_ref->m_link].m_dist : 0)) >
				...
				m_lastDotRef.m_pos = (int) m_vid->m_exData->m_unk0x08 - rdist - m_curDotRef.m_pos;
				if (idx < 0) {
					if (::Error) {
						MYERROR::Error(
							::Error,
					...
					m_lastDotRef.m_link = m_lastDotRef.m_dot->GetLink_dir(p_ref->GetAngle().m_dir + 0x80);
				}
			}
		}
	}
...
inline static SPRITE* GoalOf(SPRITE* p_sprite)
{
...
float ENGINE::Clash(ENGINE* p_other, int p_dir)
{
	...
	if (impactSpeed > Const->m_minMoveSpeed && (((m_flag & 0x7c) == 0x6c && p_other->InTrain(GoalOf(this))) ||
		...
		for (engine = p_other->m_prevEngine; engine && falloff >= 2; engine = engine->m_prevEngine) {
			falloff /= 2;
...
void ENGINE::MT_PullCoordinates(R_POS* p_ref, float p_speed, int p_deceleration)
{
	...
	for (ENGINE* e = this; e; e = e->m_nextEngine) {
		...
		e->PullTail(p_ref);
		if (e->m_nextEngine) {
			*p_ref = e->m_nextEngine->m_curDotRef;
			R_DOT* lastDot = e->m_lastDotRef.m_dot;
			int lastIdx = e->m_lastDotRef.m_link;
			SetRef(
				&e->m_nextEngine->m_curDotRef,
				lastDot->m_links[lastIdx].m_dot,
				lastDot->m_links[lastIdx].m_dist,
				e->m_lastDotRef.m_dot->m_links[lastIdx].m_backLink,
				e->m_lastDotRef.m_pos
			);
		}
		e->CalcCoor();
		if ((e->m_unk0xe0 != 0.0f || e->m_unk0xe4 != 0.0f) && (float) fabs(e->m_unk0xe0 - e->X()) > 30.0f) {
			MYERROR::Log(
				::Error,
	...
	if (p_speed != 0.0f) {
		...
		for (SPRITE* s =
				 Hash->FirstInBox(x - vid->m_unk0x384, y - vid->m_unk0x388, x + vid->m_unk0x384, y + vid->m_unk0x388);
			 s;
			 s = Hash->NextInBox()) {
			if (s == lead) {
				continue;
			...
			if ((float) fabs(s->m_x - x) < itsVid->m_unk0x384 + myVid->m_unk0x384 &&
				(float) fabs(s->m_y - y) < itsVid->m_unk0x388 + myVid->m_unk0x388 && itsVid->m_unk0x24 + s->m_z >= z &&
				z + myVid->m_unk0x24 >= s->m_z && (itsVid->m_flag & 0x4000)) {
				s->Action(85, 5, 0, 0);
			}
		}
	}
...
void ENGINE::DeleteAttackToEngine()
{
	...
	while (sprite) {
		if (sprite->m_goal == this) {
			if (sprite->m_vid->m_sprClass == 21 && ((sprite->m_flag & 0x7c) == 112 || (sprite->m_flag & 0x7c) == 116)) {
			...
			if (owner) {
				if (owner->m_child) {
					...
					if (vid == owner->m_vid->m_unk0x5c && vid->m_weaponVid && vid->m_weapon) {
						float d = NearDistanceTo(
							goal->m_x - m_commandOwner->m_child->m_x,
...
inline static void EngineBackLink(const R_DOT_REF* p_ref, int* p_back)
{
	if (p_ref->m_dot) {
...
void ENGINE::MoveEngineTact()
{
	...
	ENGINE* scan = this;
	for (;;) {
		if (scan->m_curDotRef.m_dot && scan->m_curDotRef.m_dot->m_unk0x0c == scan->m_curDotRef.m_link &&
		...
		if (scan->m_curDotRef.LinkedDot()) {
			...
			if (scan->m_curDotRef.LinkedDot()->m_unk0x0c == back) {
				if (m_speed != 0.0f) {
					if ((m_flag & 0x7c) == 0x5c &&
						(!m_noPathLinks ||
						 m_curDotRef.m_dot->m_links[m_pathLinks[0]].m_dot == m_curDotRef.LinkedDot())) {
	...
	if (!m_unk0xb4) {
		...
		if (!(m_flag & 0x80)) {
			int cmd = m_flag & 0x7c;
			if ((cmd == 112 || cmd == 116) && !(GameRand() % 5) && ReachTheTarget() != 1) {
				StartMove();
			}
		}
	...
	if (headDot->m_unk0x04 && speed > Const->m_unk0x18) {
		if (m_vid->m_idx != 85) {
			speed = Const->m_unk0x18;
		}
		Map->CreateSprite(Map->Vid(588), GetX(), GetY(), GetZ(), ANGLE((unsigned char) 0), (SPRITE*) this);
	}
	...
	if (headDot->m_unk0x04) {
		float by = headDot->m_y;
		float bx = headDot->m_x;
		for (SPRITE* s = Hash->FirstInBox(bx - 100.0f, by - 60.0f, bx + 100.0f, by + 60.0f); s; s = Hash->NextInBox()) {
...

	{
		if (m_curDotRef.LinkedDot()->m_noLinks < 2) {
			...
			if (cmd != 92 ||
				(m_noPathLinks && headDot->m_links[(unsigned char) m_pathLinks[0]].m_dot != m_curDotRef.LinkedDot())) {
				Stop();
			}
			else {
				SetCommandToTrain(0, (SPRITE*) 0, (R_DOT*) 0, (R_DOT*) 0);
	...
	{
		int stepped = m_curDotRef.DoStep((R_DOT*) m_commandDot, goal, this);
		MAN* flagman = Map->Flagman(Map->m_curArmy);
		...
		int commandArrived = (m_commandDot && m_curDotRef.LinkedDot() == m_commandDot) || !stepped ||
		...
		if (commandArrived) {
			...
			if (cmd == 112 || cmd == 116) {
				...
				if (nowDot->m_noLinks < 2 || linked->m_noLinks < 2) {
					if (cmd != 92 ||
						(m_noPathLinks && nowDot->m_links[(unsigned char) m_pathLinks[0]].m_dot != linked)) {
						Stop();
					}
					else {
						SetCommandToTrain(0, (SPRITE*) 0, (R_DOT*) 0, (R_DOT*) 0);
				...
				else if (linked->m_busyEngine && !((ENGINE*) linked->m_busyEngine)->InTrain(m_goal)) {
					Stop();
				}
			}
		}
...
intersect:
	MT_IntersectingProcessing(ref, &speed);
	...
	if (m_prevEngine && m_prevEngine->m_nextEngine != this) {
		if (::Error) {
			MYERROR::Error(
				...
				(int) (decomp_intptr) m_prevEngine->m_nextEngine,
				m_vid ? m_vid->m_idx : -1
			);
		}
		m_prevEngine->m_nextEngine = this;
	...
	if (m_nextEngine && m_nextEngine->m_prevEngine != this) {
		if (::Error) {
			MYERROR::Error(
				...
				(int) (decomp_intptr) m_nextEngine->m_prevEngine,
				m_vid ? m_vid->m_idx : -1
			);
		}
		m_nextEngine->m_prevEngine = this;
