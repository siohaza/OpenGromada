...
decomp_intptr BUILDING::Action(int p_action, decomp_intptr p_a, decomp_intptr p_b, decomp_intptr p_c)
{
	switch (p_action) {
	case 0x49:
	...
	case 0x23: {
		...
		if ((p_a > 0 && 0 != (m_flag & 0x7c)) || p_a < 0 || p_a >= Map->m_noVid || !(v = Map->m_vids[p_a])) {
			return 0;
		}
		m_buildVid = v;
		m_unk0x94 = p_b;
	...
	case 0x82: {
		if (m_ani < 0xf) {
			...
			if (!Hash->CanPlace(m_buildVid, fx, fy, fz)) {
				...
				if (s) {
					Action(0x4b, (decomp_intptr) s, 0, 0);
				}
				m_buildVid = 0;
				ChangeAnimation(0);
			}
