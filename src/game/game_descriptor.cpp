...
const GAME_DESCRIPTOR g_games[] = {
	...
	{GAME_CRAZY_LUNCH,
	 ...
	 false},
	{GAME_LAST_HOPE,
	 "AlienShooterLastHope",
	 "Alien Shooter - Last Hope",
	 "com.sigmateam.alienshooterlasthope",
	 612,
	 0x800,
	 640,
	 480,
...
bool ReadResource()
{
	...
	std::string path =
		Platform_IsAbsolutePath(g_resource.c_str()) ? g_resource : std::string(Platform_BasePath()) + g_resource;
	FILE* file = Platform_FOpen(path.c_str(), "rb");
	if (!file) {
		return Fail("file not found under the selected data directory");
	}
	...
	if (memcmp(bytes.data(), "RES ", 4) != 0) {
		return Fail(memcmp(bytes.data(), "RIFF", 4) == 0 ? "RIFF DATA has no established legacy table adapter"
														 : "expected RES container");
	}
	if (memcmp(bytes.data() + 8, "DATA", 4) != 0) {
		return Fail("expected DATA container", "DATA", 0, 8);
	}
	size_t declaredEnd = size_t(Little32(bytes, 4)) + 8;
	if (declaredEnd < 12 || declaredEnd > bytes.size()) {
		return Fail("declared container exceeds file bounds", "DATA", 0, 4);
	}
	...
	for (size_t offset = 12; offset < end;) {
		if (g_resourceView.m_sections.size() >= 1024) {
			return Fail("too many sections for the legacy table reader", "DATA", 0, offset);
		}
		if (end - offset < 8) {
			return Fail("truncated section header", "DATA", 0, offset);
		}
		std::string type((const char*) bytes.data() + offset, 4);
		size_t length = Little32(bytes, offset + 4);
		size_t begin = offset + 8;
		if (length > end - begin) {
			return Fail("section exceeds container bounds", type, 0, offset + 4);
		}
		size_t sectionEnd = begin + length;
		if (g_resourceView.m_sections.count(type)) {
			return Fail("duplicate resource section", type, 0, offset);
		}
		SECTION& section = g_resourceView.m_sections[type];
		if (length < 4) {
			return Fail("truncated section options/count", type, 0, begin);
		}
		uint32_t options = Little32(bytes, begin);
		uint32_t count = options;
		begin += 4;
		if (options & 0x80000000u) {
			if (sectionEnd - begin < 8) {
				return Fail("truncated extended section header", type, 0, begin);
			}
			if ((options & 0x100u) || Little32(bytes, begin) != 0) {
				return Fail("compressed tables require an established decoder", type, 0, begin - 4);
			}
			if ((options & 0xffu) != 0) {
				return Fail("unsupported section version", type, 0, begin - 4);
			}
			count = Little32(bytes, begin + 4);
			begin += 8;
		}
		if (count > (sectionEnd - begin) / 4) {
			return Fail("record count exceeds section bounds", type, 0, begin);
		}
		if (count > 65536 - totalRecords) {
			return Fail("too many records for the legacy table reader", type, 0, begin);
		}
		totalRecords += count;
		for (uint32_t index = 0; index < count; ++index) {
			if (sectionEnd - begin < 4) {
				return Fail("truncated record length", type, index, begin);
			}
			size_t recordLength = Little32(bytes, begin);
			begin += 4;
			if (recordLength > sectionEnd - begin) {
				return Fail("record exceeds section bounds", type, index, begin - 4);
			}
			section.m_records.push_back({begin, recordLength});
			begin += recordLength;
		}
		if (begin != sectionEnd) {
			return Fail("unaccounted bytes after the declared records", type, count, begin);
		}
		offset = sectionEnd;
		if ((length & 1) && offset < end) {
			++offset;
			++padding;
		}
	}
	if (declaredEnd != end && declaredEnd + padding != end) {
		return Fail("outer size disagrees with sections and their alignment", "DATA", 0, 4);
	}
	for (const char* type : {"OBJ ", "WEAP", "CNST", "SFX "}) {
		auto it = g_resourceView.m_sections.find(type);
		if (it == g_resourceView.m_sections.end() || it->second.m_records.empty()) {
			return Fail("required nonempty section is missing", type);
		}
	}
	return true;
}
	...
	for (const char* type : {"OBJ ", "WEAP", "CNST", "SFX "}) {
		...
		for (size_t index = 0; index < records.size(); ++index) {
			...
			if (!strcmp(type, "OBJ ")) {
				...
				size_t fixed = p_game.m_objSchema == GAME_OBJ_ZS1        ? 700
							   : p_game.m_objSchema == GAME_OBJ_LOCOLAND ? 624
																		 : 692;
			...
			else if (!strcmp(type, "WEAP")) {
				valid = record.m_size == (size_t) p_game.m_weapRecordBytes;
			}
			else if (!strcmp(type, "CNST")) {
				valid = records.size() == 1 && record.m_size == 104;
			}
			else {
				...
				if (valid) {
					valid = bytes[cursor + (prefix == 1 ? 0 : 4)] <= 100;
				}
				if (valid && prefix == 9) {
				...
				if (valid) {
					cursor += prefix;
					for (int i = 0; i < strings && valid; ++i) {
						valid = SkipString(bytes, cursor, end);
					}
					valid = valid && cursor == end;
				}
			}
			if (!valid) {
				return p_explain ? Fail(std::string("does not match profile '") + p_game.m_profileId + "'",
										type,
										index,
										record.m_offset)
								 : false;
...
const GAME_DESCRIPTOR* ByName(const char* p_name)
{
	...
	for (const auto& game : g_games) {
		if (name == Normalize(game.m_profileId) || name == Normalize(game.m_className) ||
			name == Normalize(game.m_title)) {
			return &game;
		}
	}
	if (name == "steamland") {
...
{
	return g_resource.c_str();
}
const char* Game_StartMap()
{
	return g_startMap.c_str();
}
const char* Game_Edition()
{
	return g_edition.c_str();
}
...
bool Game_Detect()
{
	...
	if (!g_configOverride.empty()) {
		...
		if (!ConfigExists(g_config)) {
			g_error = "Explicit configuration '" + g_config + "' was not found.";
		}
	}
	else if (ConfigExists("game.cfg")) {
		g_config = "game.cfg";
	}
	else {
		...
		auto findConfigs = [&](const GAME_DESCRIPTOR& game) {
			...
			if (candidate) {
				g_candidates.push_back(&game);
			}
		};
		if (explicitGame) {
			findConfigs(*explicitGame);
		}
		if (!configCount) {
			for (const auto& game : g_games) {
				findConfigs(game);
	...
	if (g_error.empty()) {
		...
		if (!g_config.empty()) {
			std::string path =
				Platform_IsAbsolutePath(g_config.c_str()) ? g_config : std::string(Platform_BasePath()) + g_config;
			...
			if (!configGame) {
				...
				for (const auto& game : g_games) {
					if (!SDL_strcasecmp(leaf.c_str(), game.m_configName) ||
						(game.m_configAlias && !SDL_strcasecmp(leaf.c_str(), game.m_configAlias))) {
						configGame = &game;
					}
				}
			}
			if (config.Get("common", "Steam")) {
				g_edition = "steam";
			}
		}
		if (ReadResource()) {
			for (const auto& game : g_games) {
				if (MatchSchema(game, false)) {
					g_candidates.push_back(&game);
				}
			}
			const GAME_DESCRIPTOR* pick = explicitGame ? explicitGame : configGame;
			if (pick) {
				if (!MatchSchema(*pick, true)) {
					pick = nullptr;
				}
			}
			else if (g_candidates.size() == 1) {
				pick = g_candidates[0];
			}
			else if (g_candidates.empty()) {
				g_error = "No implemented AS1-family resource schema matches these tables. AS2-family games are "
						  "outside this runtime.";
			}
			else {
				g_error = "Ambiguous game data; select a title with --game or supply its configuration with --config.";
			}
			if (pick) {
				int engine =
					config.GetInt("game",
								  "EngineVersion",
								  config.GetInt("common", "EngineVersion", config.GetInt("", "EngineVersion", 0)));
				if (engine && engine != pick->m_engineVersion) {
					g_error = "Configuration EngineVersion=" + std::to_string(engine) + " contradicts profile '" +
							  pick->m_profileId + "'.";
				}
				else {
					GameDesc = pick;
	...
	if (!g_detected && !g_probeJson) {
		...
		if (!g_candidates.empty()) {
			message += "\nCandidates:";
			for (const auto* game : g_candidates) {
				message += std::string(" ") + game->m_profileId;
			}
		}
		fprintf(stderr, "%s\n", message.c_str());
...
{
	return GameDesc && GameDesc->m_runtimeEnabled && (!GameDesc->m_nativeMoviePlayback || MoviePlayer::Available());
}

void Game_PrintProbeJson()
{
	printf("{\"detected\":%s,\"profile\":%s,\"edition\":%s,\"engineVersion\":%d,\"status\":%s,\"runtimeEnabled\":%s,"
		   "\"dataDirectory\":%s,\"config\":%s,\"resource\":%s,\"startMap\":%s,\"candidates\":[",
		   g_detected ? "true" : "false",
		   g_detected ? Json(GameDesc->m_profileId).c_str() : "null",
		   Json(g_edition).c_str(),
		   g_detected ? GameDesc->m_engineVersion : 0,
		   g_detected ? Json(GameDesc->m_status).c_str() : "null",
		   g_detected && Game_RuntimeAvailable() ? "true" : "false",
		   Json(Platform_BasePath()).c_str(),
		   Json(g_config).c_str(),
		   Json(g_resource).c_str(),
		   Json(g_startMap).c_str());
	for (size_t i = 0; i < g_candidates.size(); ++i) {
		printf("%s%s", i ? "," : "", Json(g_candidates[i]->m_profileId).c_str());
	}
	printf("],\"sections\":{");
	bool first = true;
	...
	printf("},\"moviePlaybackAvailable\":%s,\"rendererRequested\":%s,\"gpuDriverRequested\":%s,\"rendererSelected\":"
		   "null,\"error\":%s}\n",
		   MoviePlayer::Available() ? "true" : "false",
		   Json(Settings_RendererName()).c_str(),
		   Settings_GPUDriver() ? Json(Settings_GPUDriver()).c_str() : "null",
