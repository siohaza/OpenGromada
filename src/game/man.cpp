...
void MAN::MoveTact()
{
	...
	if (m_goal && (m_flag & 0x7c) == 4) {
		...
		char glide = (char) t;
		Rotate(GlideDirection(DirectionTo(Goal()) + (ANGLE&) glide), CurrentTime - PrevCurrentTime);
...
static inline int HaveItemFromEnd(EX_SPRITE_DATA* p_ex, int p_item)
{
...
// STUB: ALIEN 0x454800
decomp_intptr MAN::Action(int p_action, int p_a, int p_b, int p_c)
{
	switch (p_action) {
	case 0x25: {
		SPRITE* c = m_child;
		...
		VID* vid = m_vid;
		VID* cv = c->m_vid;
		if (cv != vid->m_unk0x5c || !cv->m_weaponVid || !cv->m_unk0x40 ||
			(unsigned int) c->m_unk0x50 > 0x1388 || (c->m_ani == 8 && c->m_noCadr <= c->m_endCadr))
			break;
		float fx = (float) p_a;
		...
		float gz;
		if (sx >= Graph->m_viewXMin && sx < Graph->m_viewXMax && sy >= Graph->m_viewYMin &&
			sy < Graph->m_viewYMax) {
			gz = (float) (*((unsigned short*) Graph->m_zbuffer + (int) sy * Graph->m_unk0x250 + (int) sx) >> 3) -
				 128.0f;
			if (gz > m_z + 70.0f)
				gz = m_z + 50.0f;
		}
	...
	case 0x55: {
		...
		if (dmg >= m_unk0x54 && Action(0x38, 230, 0, 0)) {
			...
			Map->CreateSprite(Map->Vid(181), (float) GetX(), (float) GetY(),
				(float) (GetZ() + 22.0f), ANGLE(0), this);
			break;
		}
		}
	...
	case 0x3e: {
		...
		if (m_exData && HaveItemFromEnd(m_exData, 204)) {
			Child()->AddLink(Map->CreateSprite(Map->Vid(200), (float) GetX(), (float) GetY(),
				(float) (GetZ() + GetVid()->m_unk0x54), Direction(), 0));
		}
		else if (m_exData && HaveItemFromEnd(m_exData, 205)) {
			Child()->AddLink(Map->CreateSprite(Map->Vid(201), (float) GetX(), (float) GetY(),
				(float) (GetZ() + GetVid()->m_unk0x54), Direction(), 0));
		}
		else if (m_exData && HaveItemFromEnd(m_exData, 206)) {
			Child()->AddLink(Map->CreateSprite(Map->Vid(202), (float) GetX(),
				(float) GetY(), (float) (GetZ() + GetVid()->m_unk0x54), Direction(), 0));
		}
		VID* v = m_vid;
