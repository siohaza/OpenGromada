...
int NativeMinimumArgs(unsigned int p_operation)
{
	...
	case 102:
		return steamAS1 ? 2 : 1;
	case 134:
		return Game_IsZS1() ? 1 : 0;
	case 136:
		return Game_IsZS1() ? 3 : 2;
	case 150:
		return steamAS1 ? 3 : 0;
	case 212:
		return steamAS1 ? 1 : 2;
	case 231:
		return Game_IsZS1() ? 1 : 4;
	case 232:
		return Game_IsZS1() ? 0 : -1;
	case 78:
	case 255:
		return Game_IsZS1() ? 5 : -1;
	...
	case 223:
		return steamAS1 ? 0 : -1;
	case 213:
		return steamAS1 || GameDesc->m_unitCountLayers > 0 ? 0 : -1;
	case 69:
	case 75:
	case 76:
	case 77:
...
int LOGIC::CallFunction(int p_fn, const SPRITE* p_a, const SPRITE* p_b, int p_c)
{
	...
	if (entry.m_extra < 0 || entry.m_type < 0 || entry.m_type > m_stack.m_n ||
		entry.m_extra > m_stack.m_n - entry.m_type) {
	...
	while (instruction != m_stackPos && !m_runtimeFault) {
		...
		int operandBytes =
			operation == 39 ? 5
			: (operation == 1 || operation == 24 || operation == 25 || operation == 28 || operation == 29 ||
			   operation == 30 || (operation >= 32 && operation <= 38) || operation == 44 || operation == 45)
				? 4
				: 0;
		...
		int needed = operation >= 6 && operation <= 23 ? 2
					 : (operation == 3 || operation == 4 || operation == 5 || operation == 24 || operation == 26 ||
						operation == 29 || operation == 38 || operation == 39 || operation == 40 || operation == 44 ||
						operation == 45)
						 ? 1
						 : 0;
		if (emptyReturn && operation == 24 && stackCount == frameBase && GameDesc->UsesLegacyEmptyReturnCondition()) {
			LOGICSTACK condition(((LOGICSTACK*) m_stack.m_data)[frameBase - 1]);
			m_stack.Insert(condition);
			stackCount = m_stack.m_n;
		}
		emptyReturn = false;
		...
		if (operation <= 23 && operation >= 6) {
			...
			const bool characterAssignment = operation == 38 && indexActive && (stack[encodedVariable].m_type & 1) &&
											 !(stack[encodedVariable].m_type & 4);
			...
			case 29: {
				...
				if (!condition->Int()) {
					...
					LOGIC_BYTECODE::WriteInt32(m_stackData + statementStart + 1,
											   LOGIC_BYTECODE::ReadInt32(m_stackData + instruction) + instruction -
												   statementStart - 1);
					instruction += 4;
				}
				statementStart = instruction;
			...
			case 30:
				if (LOGIC_BYTECODE::ReadInt32(m_stackData + instruction) < 0) {
					...
					for (const auto& fixup : m_protoFixups) {
						if (fixup.first == instruction) {
							name = m_variables.m_data[fixup.second].m_name.m_str;
						}
					}
					RuntimeError(name, -1);
					break;
