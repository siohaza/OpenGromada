...
inline static unsigned int AniTactTime(unsigned int p_dur)
{
	return CurrentTime - PrevCurrentTime > p_dur ? CurrentTime - PrevCurrentTime : p_dur;
}

inline static void PtrSpriteAssign(PTR_SPRITE* p_dst, SPRITE* p_src)
{
	if (p_src) {
		++p_src->m_noRef;
	}
...
inline static void CommitMove(CIV_ROBOT* p_robot, float p_x, const float& p_y, const float& p_z)
{
	p_robot->ChangeCoor(p_x, p_y, p_z);
}

// GLOBAL: ALIEN 0x484930
int CIV_ROBOT::RobotBuildingVids[16] = {170, 803, 805, 806, 807, 809, 810, 813, 846, 848, 861, 863, 864, 874, 875, 895};
...
CIV_ROBOT::CIV_ROBOT(VID* p_vid, float p_x, float p_y, float p_z, ANGLE p_dir, SPRITE* p_parent)
	: CREATURE(p_vid, p_x, p_y, p_z, p_dir, p_parent), m_target()
{
	m_state = 0;
	m_target = 0;
	m_unk0xa0 = 0;
...
{
	SPRITE* result = 0;
	for (SPRITE* i = Hash->FirstInBox(X() - 200, Y() - 133, X() + 200, Y() + Z() + 133); i; i = Hash->NextInBox()) {
		VID* vid = i->m_vid;
		if ((vid->m_unk0x0c & 2) && vid->m_sprClass == 1) {
			if (IsRobotBuilding(i)) {
				SPRITE* region;
				if (!m_region || ((region = FindRegion(i->X(), i->Y())) != 0 && m_region->m_vid == region->m_vid)) {
					result = i;
					if (GameRand() % 4 == 0) {
						break;
					}
...
void CIV_ROBOT::PathIsBlocked()
{
	m_speed = 0;
	if (!m_turn) {
		m_turn = (GameRand() % 2) ? 10 : -10;
	}
	if ((m_flag & 0x7c) == 4 && !(GameRand() % 9)) {
		Stop();
		m_state = 0;
	}
}
...
void CIV_ROBOT::MoveTact()
{
	...
	if (z - Z() > m_vid->m_unk0x64 || Z() - z > m_vid->m_unk0x68) {
		goto blocked;
	}
	...
	if (x >= 0.0f && x < Map->m_w && y >= 0.0f && y < Map->m_h) {
		...
		if (IsXYCross(blocker->m_vid, blocker->m_x, blocker->m_y)) {
			if ((bz = blocker->m_z, m_vid->m_unk0x24 + m_z >= bz) && bz + blocker->m_vid->m_unk0x24 >= m_z) {
				ChangeCoor(x, y, z);
				return;
			}
		}
		...
		if (blocker->m_vid == m_vid && (m_state == 15 || ((CIV_ROBOT*) blocker)->m_state == 15)) {
...
void CIV_ROBOT::ChangeAnimation(int p_ani)
{
	...
	if (child && child->m_vid == m_vid->m_unk0x5c && child->m_ani != p_ani && child->m_noCadr >= child->m_endCadr) {
...
decomp_intptr CIV_ROBOT::Action(int p_action, decomp_intptr p_a, decomp_intptr p_b, decomp_intptr p_c)
{
	switch (p_action) {
	case 9: {
		if (m_ani == 8) {
	...
	case 85:
		if (p_a > 0) {
			for (SPRITE* i = Hash->FirstInBox(X() - 150.0f, Y() - 150.0f, X() + 150.0f, Y() + 300.0f); i;
				 i = Hash->NextInBox()) {
	...
	case 130: {
		...
		if (m_unk0xa4) {
			...
			if (m_state == 14) {
				...
				if (GameRand() % 5) {
					...
					m_state = GameRand() % 3 ? 12 : 10;
				}
			}
			else if (m_target.m_ptr && ((m_state != 9 && !(GameRand() % 3)) || !(GameRand() % 3))) {
				m_state = 9;
			}
			else if (m_target.m_ptr && !(GameRand() % 2)) {
				m_state = 11;
			}
			else if (m_state == 5 && GameRand() % 3) {
				m_state = 5;
			}
			else if (m_state == 1 && GameRand() % 6) {
				m_state = 1;
			}
			else if (m_state == 4 && !(GameRand() % 2)) {
				m_state = 4;
			}
			else if (!(GameRand() % 7)) {
				m_state = 4;
		...
		case 5:
			...
			if (m_ani == 2 && !(GameRand() % 3)) {
				Rotate(Direction() - 32, AniTactTime(m_vid->m_aniDuration[m_ani]));
			}
			else if (m_ani == 2 && !(GameRand() % 3)) {
				Rotate(Direction() + 32, AniTactTime(m_vid->m_aniDuration[m_ani]));
			}
			else {
				ChangeAnimation(2);
			...
			break;
		case 14:
			if (m_goal || (m_actions.m_n && m_actions.m_data[m_actions.m_n - 1].m_cmd != 73)) {
				if (!(m_flag & 0x80)) {
					if (!m_actions.m_n || m_actions.m_data[m_actions.m_n - 1].m_cmd == 73) {
						m_state = 15;
					}
				...
				if (b) {
					...
					float bx = b->m_x;
					float s = ANGLE::SinTable[b->m_dir];
					SPRITE* beacon = new SPRITE(EmptyVid, bx - s * 32.0f, c * 32.0f + by, bz, ANGLE(0), 0);
					Move(beacon);
					m_actions.InsertFirst(ACT(34, (decomp_intptr) b, 0, 0));
					break;
				}
				m_unk0xa4 = 1;
				break;
		...
		case 7: {
			...
			if (m_target.m_ptr) {
				Rotate(DirectionTo(m_target.m_ptr).Invert(), AniTactTime(vid->m_aniDuration[m_ani]));
			}
			SPRITE* child = m_child;
			if (!child || child->m_vid != m_vid->m_linkVid) {
				break;
		...
		case 9: {
			...
			if (m_target.m_ptr) {
				Child()->Rotate(DirectionTo(m_target.m_ptr), AniTactTime(m_vid->m_aniDuration[m_ani]));
			}
			else {
				Child()->Rotate(Direction(), AniTactTime(m_vid->m_aniDuration[m_ani]));
			}
