...
inline ANGLE::ANGLE(float p_x, float p_y, int* p_dist)
{
	AngleAssign(this, Decart2Polar((int) p_x, (int) p_y, p_dist));
}
...
char* SPRITE::Insert()
{
	...
	if (child)
		child->Insert();
	return Map->InsertSpriteToLayer(this);
}
...
void SPRITE::ChangeAnimation(int p_ani)
{
	if (p_ani >= 0x11) {
		MYERROR::Error(::Error, "SPRITE %i", 4,
			...
			"new_animation in ChangeAnimation", p_ani, m_vid ? m_vid->m_idx : -1);
		return;
	}
	SPRITE* child = m_child;
	if (child) {
		...
		if (v == m_vid->m_unk0x5c) {
			if ((!v->m_unk0x40 && !(v->m_flag & 0x1000) && child->m_ani != 8 && child->m_ani < 0xf) ||
				p_ani == 0xf || p_ani == 0x10)
...
static __forceinline int InterpolateGammaCurve(const int* p_values, float p_phase)
{
	...
	if (frame >= 7)
		...
		result = (int) ((p_values[frame + 1] - p_values[frame]) * (p_phase - frame)
			+ p_values[frame]);
	return result;
}
...
static inline int SpriteMaxHp(const SPRITE* p_sprite)
{
...
{
	if (m_child && m_child->m_vid == m_vid->m_unk0x5c && m_child->m_vid->m_weaponVid &&
		m_child->m_vid->m_unk0x40) {
...
static inline ANGLE DirDiff(const unsigned char* p_dir, unsigned char p_target)
{
...
static inline int CanDescendFightLink(const SPRITE* p_sprite)
{
	SPRITE* c = p_sprite->m_child;
	return c && c->m_vid == p_sprite->m_vid->m_unk0x5c && c->m_vid->m_weaponVid &&
		c->m_vid->m_unk0x40;
}

static inline ANGLE DirectionOf(const SPRITE* p_from, const SPRITE* p_to)
{
...
static inline float NearDistanceOf(float p_x, float p_y)
{
	float x = (float) fabs(p_x);
	float y = (float) fabs(p_y);
	if (x > y)
...
static inline int IsNotCloserEnemy(const float& p_dist, const float& p_bestDist)
{
	return p_dist >= p_bestDist;
}

static inline SPRITE* LastHashSprite(HASH_MAP* p_hash, unsigned int p_n)
{
...
SPRITE* SPRITE::SeekEnemy()
{
	...
	while (c) {
		...
		if ((cv->m_unk0x0c & mask) && cv->m_defaultMaxHp &&
			(((c->m_flag ^ s->m_flag) & 0x1800) || (s->m_vid->m_exData->m_unk0x04 & 0x80)) &&
			!(cv->m_flag & 0x20000) &&
			...
			if (ok) {
				...
				if (dist <= range && dist >= minRange) {
					...
					if (ok) {
						if ((s->m_flag & 0x1800) || c->m_vid->m_sprClass != 21 ||
							!((ENGINE*) c)->HaveArmy(0)) {
							bestDist = dist;
							best = c;
							if (pick) {
...
{
	Sound->PlaySFXFromCoor(p_sfx,
		m_x - Map->m_shiftX - Graph->m_width * 0.5f,
		m_y - m_z - Map->m_shiftY - Graph->m_height * 0.5f);
}
...
void SPRITE::DrawRectangle()
{
	...
	Graph->Box(Map->ScrX(m_x - m_vid->m_unk0x384), Map->ScrY(m_y - m_z - m_vid->m_unk0x388),
		Map->ScrX(m_vid->m_unk0x384 + m_x), Map->ScrY(m_y - m_z + m_vid->m_unk0x388), color);
	if (m_vid->m_unk0x0c & 6)
		Graph->Line(ScreenX(), Map->ScrY(m_y - m_z - m_vid->m_unk0x24), ScreenX(), ScreenY(),
			GRAPH_CORE::BLUE);
	GRAPH_CORE::PrintfXY((GRAPH_CORE*) Graph, m_x - Map->m_shiftX + 1.0f,
		m_y - m_z - Map->m_shiftY,
		"%i", m_vid->m_idx);
}

static inline float CreateLinkXOffset(const unsigned char& p_dir, const VID* p_vid)
{
	return ANGLE::CosTable[p_dir] * p_vid->m_unk0x4c
		- ANGLE::SinTable[p_dir] * -p_vid->m_unk0x50;
}

static inline float CreateLinkYOffset(const unsigned char& p_dir, const VID* p_vid)
{
	return ANGLE::SinTable2[p_dir] * p_vid->m_unk0x4c
		- ANGLE::CosTable2[p_dir] * p_vid->m_unk0x50;
}
...
void SPRITE::CreateLink()
{
	...
	AddLink(Map->CreateSprite(m_vid->m_unk0x5c, xoff + m_x, yoff + m_y, m_vid->m_unk0x54 + m_z,
		(ANGLE&) m_dir, 0));
	if (m_child)
		m_child->ChangeArmy((m_flag >> 11) & 3);
	else
		MYERROR::Error(::Error, "SPRITE %i", 3,
			// STRING: ALIEN 0x48458c
			"link", 0, m_vid ? m_vid->m_idx : -1);
}
...
STRING SPRITE::GetTextActions() const
{
	...
	for (int i = 0; i < m_actions.m_n; ++i)
		...
		result += Printf("%c%i,%i,%i;", m_actions.m_data[i].m_cmd + 60,
...
void SPRITE::SetTextActions(const STRING& p_text)
{
	...
	while (strcmp(buffer.m_str, empty_str)) {
		...
		sscanf(buffer.m_str, "%c%i,%i,%i", &parsed.command,
			&parsed.a, &parsed.b, &parsed.c);
		m_actions.Insert(ACT(parsed.command - 60, parsed.a,
			parsed.b, parsed.c));
		// STRING: ALIEN 0x481a10
		buffer = *(STRING*) buffer.After(&parsed.after, ";");
...
void SPRITE::AddActionAfterStop(int p_cmd, int p_a, int p_b, int p_c)
{
...
int SPRITE::CanAttackThisSprite(SPRITE* p_sprite)
{
	...
	if (vid->m_weaponVid && vid->m_unk0x40 &&
		(vid->m_exData->m_unk0x00 & p_sprite->m_vid->m_unk0x0c))
		return 1;
	SPRITE* c = m_child;
...
int SPRITE::Attack(SPRITE* p_target)
{
	...
	if (!p_target || p_target->m_vid == EmptyVid || CanAttackThisSprite(p_target) ||
		m_vid->m_sprClass == 5 || Action(0x9f, 0, 0, 0)) {
		SPRITE* target = p_target;
		if (m_child && m_child->m_vid == m_vid->m_linkVid)
			*(SPRITE* volatile*) &p_target = (SPRITE*) m_child->m_vid;
		SetCommand(3, target);
	}
	return 0;
}
...
int SPRITE::GetFireDamage()
{
	...
	for (SPRITE* s = this; s; s = s->m_child) {
		VID* vid = s->m_vid;
		if (*(VID* volatile*) &vid->m_weaponVid && vid->m_unk0x40)
			total += (*(VID* volatile*) &vid->m_weaponVid)->GetFireDamage();
	}
	return total;
}
...
static __forceinline void ListIntInsert(LIST_INT* p_list, int p_value)
{
	...
	if (p_list->m_n >= max) {
		...
		if (newMax > max) {
			...
			p_list->m_data = (int*) operator new(4 * newMax);
			if (!p_list->m_data)
...
{
	if (strcmp(p_text.m_str, empty_str)) {
		char* buffer;
		// STRING: ALIEN 0x4845b0
		p_text.After(&buffer, "\1");
		if (!m_exData)
			m_exData = new EX_SPRITE_DATA(this);

		while (strcmp(buffer, empty_str)) {
			int item;
			sscanf(buffer, "%i", &item);
			ListIntInsert(&m_exData->m_list, item);

			char* after;
			*(STRING*) &buffer = *(STRING*) ((STRING*) &buffer)->After(&after, "\1");
