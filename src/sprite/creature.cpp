...
void CREATURE::MoveTact()
{
	...
	if (CanPlaceWithCrush(x, y, z)) {
		...
		if (r != 0 && (x0 = r->m_x,
					   w2 = r->m_w * 0.5f,
					   x0 - w2 > x || x > x0 + w2 || (y0 = r->m_y, h2 = r->m_h * 0.5f, y0 - h2 > y || y > y0 + h2))) {
...
decomp_intptr CREATURE::Action(int p_action, decomp_intptr p_a, decomp_intptr p_b, decomp_intptr p_c)
{
	switch (p_action) {
	default:
	...
	case 130: { // ACT_NEXT_COMMAND
		...
		if (ani < 15) {
			...
			if (m_goal && (m_flag & 0x7c) == 4) {
				...
				int cmd = m_flag & 0x7c;
				if (cmd == 12 || cmd == 16 || CurrentTime - (CurrentTime & 0x7ff) > m_tactTime) {
					...
					if (state == 1 && m_speed == 0.0f) {
						...
						if ((turret && turret->m_vid == m_vid->m_linkVid && turret->m_vid->m_aniChildVid[8] &&
							 turret->m_vid->m_weapon && turret->m_goal) ||
							m_goal) {
							SetCommand(0, 0);
						}
					}
				...
				if (cmd != 4 && cmd != 12 && cmd != 16) {
					...
					if (CurrentTime - (CurrentTime & 0x7ff) > m_tactTime) {
						if (!(GameRand() % 4)) {
							...
							m_speed == 0.0f && !(GameRand() % 3) &&
							(m_vid->m_noAnimCadr[12] || m_vid->m_unk0x20c[12] || m_vid->m_aniSfx[12])
						) {
	...
	case 85: // ACT_DAMAGE
		if (p_a > 0) {
			for (SPRITE* s = Hash->FirstInBox(X() - 150.0f, Y() - 150.0f, X() + 150.0f, Y() + 150.0f); s;
				 s = Hash->NextInBox()) {
