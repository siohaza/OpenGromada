	...
	if (p_x < p_w && p_y < p_h && p_x >= 0 && p_y >= 0) {
		...
		if (count >= 30) {
			MYERROR::Error(
				...
				"AddDotToArray() a[x][y][0]>=ARR_SIZE",
				0
			);
...
inline static int Between(int p_v, int p_a, int p_b)
{
	if (p_a >= p_b) {
...
void R_MAP::SetPushLine(int p_x1, int p_y1, int p_x2, int p_y2, int p_value)
{
	...
	if (!startDot) {
		MYERROR::Log(
			...
			"!!!ERROR!!!R_MAP: Can't found dot in %i,%i",
			p_x1,
			p_y1
		);
	}
		...
		if (n <= 0) {
			MYERROR::Log(
				...
				"!!!ERROR!!!R_MAP: Can't SetPushLine in %i,%i",
				p_x1,
				p_y1
			);
		}
			...
			if (endDot->FindNewDotWithoutBusyDots() >= 0) {
				...
				while (dot && dot != endDot) {
					for (int i = 0; i < dot->m_noLinks; ++i) {
						if (dot->m_unk0x94[0] == dot->m_links[i].m_dot->m_unk0x94[0] + 1) {
							dot->m_unk0x0c = i;
							dot->m_unk0x10 = (unsigned int) p_value;
				...
				MYERROR::Log(
					...
					"!!!ERROR!!!R_MAP: Can't PushLineFindDot in %i,%i",
					p_x1,
					p_y1
				);
			}
...
R_DOT* R_MAP::SetSemaphoreOrMine(int p_x, int p_y, int p_value, int p_d)
{
	...
	if (dot) {
		...
		if (best < 0) {
			MYERROR::Error(
				...
				"\xed\xe5\xf2\xf3 \xf1\xe2\xff\xe7\xe5\xe9 \xf3 \xf2\xee\xf7\xea\xe8 - \xf2\xe0\xea\xee\xe3\xee "
				"\xe1\xfb\xf2\xfc \xed\xe5 \xec\xee\xe6\xe5\xf2",
				0
			);
			return 0;
		}
...
static_assert(sizeof(RGB16) == 2, "RGB16 is a packed 16-bit pixel");
		...
		for (int k = 0; k < dot->m_noLinks; ++k) {
			...
			if (link->m_crossLink) {
				Graph->Line(
					sx,
					sy,
					link->m_crossLink->m_dot->GetScreenX(),
					link->m_crossLink->m_dot->GetScreenY(),
					GRAPH_CORE::GREEN
				);
			}
		}
		...
		if (i < 0) {
			...
			if (m_list.m_n >= max) {
				...
				if (newMax > max) {
					...
					m_list.m_data = (R_DOT**) operator new(sizeof(*m_list.m_data) * newMax);
					if (!m_list.m_data) {
...
inline static VID* GetVidOf(SPRITE* p_sprite)
{
