...
#define DECOMP_INLINE_LIST_ACT_SET_NUMBER
...
SPRITE::SPRITE(VID* p_vid, float p_x, float p_y, float p_z, ANGLE p_dir, SPRITE* p_parent)
{
	if (p_vid->m_flag & 0x100) {
		p_x = (float) (8 - rand() % 17) + p_x;
		p_y = (float) (8 - rand() % 17) + p_y;
	}
	m_vid = p_vid;
	m_x = p_x;
	m_y = p_y;
	...
	if (p_parent)
		...
		army = *(int*) ((char*) m_vid->m_exData + 44);
	else
		army = *(int*) ((char*) m_vid->m_linkVid->m_exData + 44);
	...
	m_goal = 0;
	army = (m_flag >> 11) & 3;
	int hidden = (m_vid->m_flag & 0x20000) != 0
		&& ((m_flag >> 11) & 3) != (int) Map->m_curArmy;
...
SPRITE::~SPRITE()
{
	...
	if (m_vid == EmptyVid && m_noRef)
		MYERROR::Error(::Error, "SPRITE %i", 4,
			...
			"noRef for SPRITE with EmptyVid", m_noRef, m_vid ? m_vid->m_idx : -1);
	if (m_noRef > 1) {
		Hash->Delete(this);
		if (m_noRef > 1)
	...
	fn = (int) m_vid;
	if (((VID*) fn)->m_entitiesNumber[army])
		--((VID*) fn)->m_entitiesNumber[army];
	SetGoal(0);
	m_unk0x6c = 0;
	while (m_child)
	...
	if (m_noRef)
		MYERROR::Error(::Error, "SPRITE %i", 10,
			...
			"Reference count non zero after delete", m_noRef, m_vid ? m_vid->m_idx : -1);
	if (m_exData)
		delete m_exData;
}
...
void SPRITE::DeletePointerToSprite(SPRITE* p_sprite)
{
	...
	for (int i = 0; i < m_actions.m_n; ++i) {
		if ((SPRITE*) m_actions.m_data[i].m_a == p_sprite
			&& (m_actions.m_data[i].m_cmd == 32
				|| m_actions.m_data[i].m_cmd == 34
				|| m_actions.m_data[i].m_cmd == 74
				|| m_actions.m_data[i].m_cmd == 150
				|| m_actions.m_data[i].m_cmd == 151
				|| m_actions.m_data[i].m_cmd == 152
...
void SPRITE::MoveTact()
{
	...
	if ((vid->m_unk0x0c & 0x200) && (vid->m_flag & 2) && z <= newGround
		&& m_z >= curGround) {
		z = m_z;
...
static inline int NearBetween(float p_value, float p_a, float p_b, float p_pad)
{
	if (p_a < p_b) {
	...
	*p_z = Z();
	if ((m_flag & 0x7c) == 4 && !(m_flag & 0x80)) {
		MYERROR::Error(::Error,
			...
			"Move without StartMove()", 0, m_vid ? m_vid->m_idx : -1);
		StartMove();
	}
	if ((m_flag & 0x7c) == 4 && !m_goal) {
		MYERROR::Error(::Error, "SPRITE %i", 10,
			// STRING: ALIEN 0x4843dc
			"Move without goal", 0, m_vid ? m_vid->m_idx : -1);
		Stop();
	}
	...
	if (m_flag & 0x80) {
		...
		if (target > m_speed) {
			VID* vid = m_vid;
			if (*(unsigned int*) &vid->m_unk0x34 == 0x497423f0) {
		...
		VID* vid = m_vid;
		if (*(unsigned int*) &vid->m_unk0x38 == 0x497423f0) {
			m_speed = 0.0f;
		} else {
			int dt = CurrentTime - PrevCurrentTime;
	...
	if (m_speed == 0.0f
		&& (Graph->m_windForce == 0.0f || !(m_vid->m_flag & 0x1000))) {
	} else if (*(unsigned int*) &m_speed == 0x497423f0) {
		SPRITE* goal = m_goal;
	...
	if (vflag & 2)
		m_unk0x24 = m_unk0x24
			- (float) (int) (CurrentTime - PrevCurrentTime) * Const->m_unk0x08;
	else if (vflag & 4)
		m_unk0x24 = m_unk0x24
			- (float) (int) (CurrentTime - PrevCurrentTime) * Const->m_unk0x0c;
...
void SPRITE::Tact()
{
	...
	if (vid->m_unk0x47c & 0xf) {
		...
		if (!aged && total) {
			...
			--stage;
			exData->m_unk0x1c = (float) stage
				+ ((float) elapsed - (float) total * stages[stage])
	...
	if (childVid) {
		if (m_vid->m_sprClass == 23 // B_REGION
			&& m_vid->m_unk0x140[m_ani] == 0.0f
			&& m_vid->m_unk0x184[m_ani] == 0.0f) {
			REGION* region = (REGION*) this;
			float w = (region->m_flag & 8) ? Map->m_w : region->m_w;
			float h = (region->m_flag & 8) ? Map->m_h : region->m_h;
			unsigned int density = (unsigned int) (h * w
				/ childVid->m_footprintWidth / childVid->m_footprintHeight);
			...
			if (CurrentTime - CurrentTime % exData->m_unk0x14 > PrevCurrentTime) {
				...
				float vy = m_unk0x24 - childVid->m_unk0x30
					+ ANGLE::CosTable[m_dir] * m_speed;
				...
				float px = (vx != 0.0f)
					? childVid->m_footprintWidth / (vx < 0.0f ? -vx : vx)
					: 30000.0f;
				float py = (vy != 0.0f)
					? childVid->m_footprintHeight / (vy < 0.0f ? -vy : vy)
					: 30000.0f;
				if (px < py)
					py = px;
	...
	if (m_flag & 0x7c) {
		...
		if (cmd < 16 && !m_goal) {
			MYERROR::Error(::Error, "SPRITE %i", 10,
				...
				"command need goal, but goal==NULL", cmd,
				m_vid ? m_vid->m_idx : -1);
			SetCommand(0, 0);
		}
	}
	...
	for (int script = m_vid->m_unk0x408[m_ani]; script >= 0;
		script = m_vid->m_unk0x408[m_ani]) {
	...
	if (childVid && !(childVid->m_flag & 0x80)) {
		if (m_ani == 8 && (m_vid->m_exData->m_unk0x04 & 0x10)) {
			...
			if (m_noCadr == mid
				|| m_noCadr == (unsigned int) ((m_vid->m_flag & 0x40000)
					? m_endCadr : m_begCadr)) {
				CreateChild();
			}
		} else if ((m_vid->m_flag & 0x10)
			|| m_noCadr == (int) ((m_vid->m_flag & 0x40000) ? m_endCadr : m_begCadr)) {
			CreateChild();
		}
	}
...
static inline int AniStepTime(const VID* p_vid, int p_ani)
{
...
static inline void ActionFloat(float* p_out, int p_value)
{
...
// STUB: ALIEN 0x440830
decomp_intptr SPRITE::Action(int p_action, int p_a, int p_b, int p_c)
{
	...
	case 35: {
		...
		if (p_b == 0 && p_c == 0) {
			...
			if (p_b < 0)
				x = m_x - (float) p_b - (float) (2 * (rand() % (1 - p_b)));
			if (p_c < 0)
				y = m_y - (float) p_c - (float) (2 * (rand() % (1 - p_c)));
		}

		SPRITE* child = Map->CreateSprite(Map->Vid(nvid), x, y, GetZ(),
			Direction(), this);
		if (!child)
			return 0;
		Action(75, (int) (decomp_intptr) child, 0, 0);
		return 0;
	}
	case 101:
	...
	case 74:
		...
		return 0;
	case 70: {
		ACT act((((m_flag >> 2) & 0x1f) << 8) + 0x4a, (int) m_goal, 0, 0);
		m_actions.Insert(act);
		if (m_goal)
			++m_goal->m_noRef;
		return 0;
	...
	case 62: { // ACT_CHANGE_VID
		...
		if (m_vid->m_sprClass != newVid->m_sprClass)
			MYERROR::Error(::Error,
				...
				"ACT_CHANGE_VID", p_a, m_vid ? m_vid->m_idx : -1);
		...
		VID* target = p_a >= 0 && p_a < Map->m_noVid && Map->m_vids[p_a]
			? Map->m_vids[p_a] : EmptyVid;
		m_vid = target;
	...
	case 91: {
		SPRITE* goal = new SPRITE(EmptyVid, (float) p_a, (float) p_b, (float) p_c,
			ANGLE(0), 0);
		SetGoal(goal);
		return 0;
	}
	...
	case 100: {
		...
		if (child) {
			...
			if (childVid == vid->m_linkVid && childVid->m_weaponVid
				&& childVid->m_weapon && vid->m_exData->m_unk0x18 == 0.0f)
	...
	case 85:
		...
		if (m_vid->m_unk0x408[18] >= 0 &&
			Map->ScriptRun(m_vid->m_unk0x408[18], this, (SPRITE*) p_b, p_a)) {
			return 0;
		}
		...
		if (m_vid->m_unk0x408[7] >= 0 &&
			Map->ScriptRun(m_vid->m_unk0x408[7], this, (SPRITE*) p_b, 0)) {
			return 0;
		}
		if (m_vid->m_aniSfx[7])
	...
	case 37: {
		int weaponType;
		if (m_child && m_child->m_vid == m_vid->m_linkVid &&
			m_child->m_vid->m_weaponVid && m_child->m_vid->m_weapon)
		...
		SPRITE* marker = new SPRITE(EmptyVid, (float) p_a, (float) p_b, gz,
			ANGLE(0), 0);
		SetCommand(4, marker);
		return 0;
	}
	case 200: {
		...
		res->Read(m_actions.m_data, 12 * m_actions.m_n);
		...
		for (int idx = 0; idx < m_actions.m_n; ++idx) {
			...
			if (act.m_cmd == 0x28) {
				...
				MYERROR::Error(::Error,
					...
					act.m_cmd, m_vid ? m_vid->m_idx : -1);
			}
			int cmd = act.m_cmd;
			if (cmd == 0x20 || cmd == 0x22 || cmd == 0x4a || cmd == 0x96 ||
				cmd == 0x97 || cmd == 0x98 || cmd == 0x4b)
				act.m_a =
					(int) (decomp_intptr) Map->m_relation.Decode(act.m_a);
		}
	...
	case 80: { // ACT_SAVE
		...
		res->Write(m_actions.m_data, 16 * m_actions.m_n);
		...
		if (m_exData) {
			...
			if (!list.m_data)
				MYERROR::LogExit(::Error,
					...
					"!!!ERROR!!!::LIST: Not enough memory for = %i", src.m_max);
	...
	case 81: { // ACT_RESTORE
		...
		if (m_actions.m_n > m_actions.m_max) {
			...
			if (!m_actions.m_data)
				MYERROR::LogExit(::Error,
					"!!!ERROR!!!::LIST: Not enough memory %i", m_actions.m_n);
			if (old) {
		...
		res->Read(m_actions.m_data, 16 * m_actions.m_n);
		...
		for (int idx = m_actions.m_n - 1; idx >= 0; --idx) {
			...
			if (cmd == 0x20 || cmd == 0x22 || cmd == 0x4a || cmd == 0x96 ||
				cmd == 0x97 || cmd == 0x98 || cmd == 0x4b) {
				act.m_a = (int) (decomp_intptr) Map->m_relation.Decode(act.m_a);
			} else if (idx && cmd == 0x49 &&
				m_actions.m_data[idx - 1].m_cmd == 0x49 && idx < m_actions.m_n) {
		...
		if (list.m_n > list.m_max) {
			...
			if (!list.m_data)
				MYERROR::LogExit(::Error,
					"!!!ERROR!!!::LIST: Not enough memory %i", list.m_n);
			if (old) {
		...
		if (list.m_n) {
			...
			if (dst.m_data)
				operator delete(dst.m_data);
			dst.m_n = list.m_n;
			dst.m_max = list.m_max;
			dst.m_data = new int[list.m_max];
			if (!dst.m_data)
				MYERROR::LogExit(::Error,
					"!!!ERROR!!!::LIST: Not enough memory for = %i", dst.m_max);
	...
	case 15: {
		...
		for (SPRITE* t = Hash->FirstInBox(m_x - rx, m_y - ry, m_x + rx, m_y + ry);
			t; t = Hash->NextInBox()) {
			if (t == this)
				continue;
			VID* tvid = t->m_vid;
			...
			if (Map->GetGroundZ_ff((t->m_x + m_x) * 0.5f, (t->m_y + m_y) * 0.5f)
				<= t->m_z + tvid->m_unk0x24) {
			if (Map->GetGroundZ_ff((m_x * 3.0f + t->m_x) * 0.25f,
					(m_y * 3.0f + t->m_y) * 0.25f) <= t->m_z + tvid->m_unk0x24) {
			if (Map->GetGroundZ_ff((t->m_x * 3.0f + m_x) * 0.25f,
					(t->m_y * 3.0f + m_y) * 0.25f) <= t->m_z + tvid->m_unk0x24) {
			...
			if ((m_vid->m_flag & 0x4000000) && m_vid->m_blastRadius != 0.0f) {
				...
				int damage = (int) (fireDamage
					- fireDamage * dist / m_vid->m_blastRadius);
				t->Action(85, damage, (int) (decomp_intptr) this, 0);
			} else {
				t->Action(85, m_vid->m_fireDamage, (int) (decomp_intptr) this, 0);
			}
			}
			}
			}
...
void SPRITE::DrawSecondaryInfo()
{
	...
	*(int*) &spriteX = *(int*) &m_x;
	...
	GRAPH_CORE::PrintfXY((GRAPH_CORE*) Graph, viewX + 22.0f, y,
		// STRING: ALIEN 0x484500
		"Ref=%-3i cmd=%1i ani=%-2i hp=%-3i AT=%i goal=%-3i spd=%-3i,%-3i timer=%i ammo=%i mvE=%1u%1u %i,%i,%i",
		m_noRef, (m_flag >> 2) & 0x1f, m_ani, hp,
		m_unk0x04, goalIdx,
		(int) (m_speed * 1000.0f), (int) (m_unk0x24 * 1000.0f),
		m_unk0x50, (int) Action(92, 0, 0, 0), (m_flag >> 14) & 1, (m_flag >> 15) & 1,
		(int) spriteX, (int) spriteY, (int) m_z);
	if (m_child && m_child->m_vid == m_vid->m_linkVid) {
		y = y + 12.0f;
		...
		GRAPH_CORE::PrintfXY((GRAPH_CORE*) Graph, childViewX + 22.0f, y,
			// STRING: ALIEN 0x4844bc
			"Ref=%-3i cmd=%1i ani=%-2i hp=%-3i AT=%i goal=%-3i timer=%i ammo=%i",
			m_child->m_noRef, (m_child->m_flag >> 2) & 0x1f, m_child->m_ani,
			hp, m_child->m_unk0x04, childGoalIdx,
			m_child->m_unk0x50, (int) m_child->Action(92, 0, 0, 0));
	}
	if (m_actions.m_n) {
		y = y + 12.0f;
		STRING text;
		...
		for (int i = m_actions.m_n - 1; i >= 0; --i) {
			text += Printf(
				...
				"%i(%i,%i,%i) ", m_actions.m_data[i].m_cmd, m_actions.m_data[i].m_a,
				m_actions.m_data[i].m_b, m_actions.m_data[i].m_c);
		}
		COLOR white;
...
{
	if (m_goal)
		Graph->Line((float) ScreenX(), (float) ScreenY(), (float) Goal()->ScreenX(),
			(float) Goal()->ScreenY(), GRAPH_CORE::GREEN);
	if (m_child && m_child->m_goal)
		Graph->Line((float) Child()->ScreenX(), (float) Child()->ScreenY(),
			(float) Child()->Goal()->ScreenX(), (float) Child()->Goal()->ScreenY(), GRAPH_CORE::RED);
}

static inline void AssignAngleValue(ANGLE* p_dst, const ANGLE& p_src)
{
	unsigned char value;
	if (p_dst != &p_src)
		value = p_src.m_dir;
...
int SPRITE::CanPlace(float p_x, float p_y, float p_z)
{
	VID* vid;
	if (!m_vid->m_unk0x18)
		return 0;
	...
	VID* myVid = m_vid;
	SPRITE* s = Hash->FirstInBox(p_x - myVid->m_unk0x384, p_y - myVid->m_unk0x388,
		p_x + myVid->m_unk0x384, p_y + myVid->m_unk0x388);
	while (s) {
		if (s != this) {
			vid = m_vid;
			if (s->m_ani < 15) {
				...
				if ((float) fabs(s->m_x - p_x) < v->m_unk0x384 + vid->m_unk0x384
					&& (float) fabs(s->m_y - p_y) < v->m_unk0x388 + vid->m_unk0x388
					&& v->m_unk0x24 + s->m_z >= p_z
					&& p_z + vid->m_unk0x24 >= s->m_z
					&& (v->m_unk0x18 & vid->m_unk0x18))
					return (int) s;
			}
		}
		s = Hash->NextInBox();
...
int SPRITE::CanPlaceWithCrush(float p_x, float p_y, float p_z)
{
	if (!m_vid->m_unk0x18)
		return 0;
	if (m_vid->m_flag & 0x200) {
...
// STUB: ALIEN 0x443c40
int SPRITE::CanPlaceWithCrushAndGlide(float* p_x, float* p_y, float* p_z)
{
	if (m_vid->m_unk0x18) {
		int blocker = CanPlace(*p_x, *p_y, *p_z);
		if (!blocker)
			goto placed;
		{
			int script = m_vid->m_unk0x408[19];
			if (script >= 0
				&& Map->ScriptRun(script, this,
					blocker == (int) Mouse ? 0 : (SPRITE*) blocker, 0)) {
				*p_x = X();
				*p_y = Y();
				*p_z = Z();
				return 0;
		...
		if (blocker != (int) Mouse
			&& (((SPRITE*) blocker)->m_vid->m_exData->m_unk0x04 & 0x40)) {
			SPRITE* other = (SPRITE*) blocker;
	...
	popDepth:
		...
		if (blocker != (int) Mouse) {
			...
			if (other->m_ani < 15) {
				VID* v = other->m_vid;
				VID* mine = m_vid;
				if (other->IsXYCross(mine, m_x, m_y)
					&& v->m_unk0x24 + other->m_z >= m_z
...
ANGLE SPRITE::GlideDirection(ANGLE p_dir)
{
	...
	if (a3 <= 0x60 || a3 >= 0xa0) {
		if (a3 > 0xe0 || a3 < 0x20) {
			...
			if (CanPlace(m_x - (hw = m_vid->m_footprintWidth * 0.5f),
					m_y - (hh = m_vid->m_footprintHeight * 0.5f), m_z)
		...
		float hw = m_vid->m_footprintWidth * 0.5f;
		float hh = m_y;
		if (CanPlace(X() - (hw = m_vid->m_footprintWidth * 0.5f),
				hh + m_vid->m_footprintHeight * 0.5f, m_z)
			&& !CanPlace(X() + (hw = m_vid->m_footprintWidth * 0.5f), m_y, m_z))
			return ANGLE((unsigned char) 88);
		if (CanPlace(m_x + hw,
				(hh = m_y) + m_vid->m_footprintHeight * 0.5f, m_z)
			&& !CanPlace(m_x - hw, m_y, m_z))
			return ANGLE((unsigned char) -88);
	}
...
void SPRITE::CreateChild()
{
	...
	ANGLE dir = (childVid->m_flag & 0x2000000) ? ANGLE(0)
		: m_vid->SteppedDirection(ANGLE(m_dir));
	...
	for (; barrel < count; ++barrel) {
		...
		if (childVid->m_sprClass == 2
			&& Hash->CanPlace(childVid, ox + m_x, oy + m_y,
				m_vid->m_unk0x1c8[m_ani] + m_z))
			continue;
		if (m_ani == 8 && !m_goal) {
			continue;
		}
		SPRITE* child = Map->CreateSprite(childVid, ox + m_x, oy + m_y,
			m_vid->m_unk0x1c8[m_ani] + m_z,
			(childVid->m_flag & 1) ? ANGLE(rand() % 256) : (ANGLE&) m_dir, this);
		if (child && m_goal && m_ani == 8) {
			...
			if ((vex->m_unk0x04 & 0x20) && m_goal->m_vid != EmptyVid) {
				...
				float ty = 0.0f;
				for (int t = 0; t < 5; ++t) {
					float randomOffset = (float) rand() * radius * 0.000061037019f;
					tx = radius + m_goal->m_x + bx - randomOffset;
					randomOffset = (float) rand() * radius * 0.000061037019f;
					ty = radius + m_goal->m_y + by - randomOffset;
					if (Map->GetGroundZ_ff(tx, ty) < m_goal->m_z) {
						if (Map->GetGroundZ_ff((m_goal->X() + tx) * 0.5f,
							(m_goal->Y() + ty) * 0.5f) < m_goal->m_z)
							break;
					}
				}
				float goalZ = m_goal->m_z;
				SPRITE* marker = new SPRITE(EmptyVid, tx, ty, goalZ,
					ANGLE(0), 0);
				child->Attack(marker);
				child->StartMove();
			}
	...
	if (m_ani == 8 && !(m_flag & 0x2000)) {
		int cmd = m_flag & 0x7c;
		if (cmd == 16 || cmd == 20) {
			if ((!(m_vid->m_flag & 0x10) && !(childVid->m_flag & 0x80))
				|| m_noCadr == m_endCadr) {
				...
				if (m_goal && m_parent && m_parent->m_goal == m_goal) {
					...
					if (m_parent->m_vid->m_sprClass == 21
						&& (m_parent->m_flag & 0x7c) == 0x74)
...
static inline int EnemyPriority(SPRITE* p_sprite)
{
...
int SPRITE::IsBetterEnemy(float p_dist, float p_bestDist, SPRITE* p_cand, SPRITE* p_best)
{
	...
	if (child && child->m_vid == m_vid->m_unk0x5c && child->m_vid->m_weaponVid &&
		child->m_vid->m_unk0x40 && ex->m_unk0x18 == 0.0f)
...
static inline int HasAttackWeapon(const VID* p_vid)
{
	return p_vid->m_aniChildVid[8] && p_vid->m_weapon;
}

static inline int IsAttackBusy(const SPRITE* p_sprite)
{
	return p_sprite->m_unk0x50 > 5000
		|| (p_sprite->m_ani == 8 && p_sprite->m_noCadr <= p_sprite->m_endCadr);
}
...
int SPRITE::AttackTact(int p_time)
{
	...
	if (child) {
		VID* childVid = child->m_vid;
		if (childVid == m_vid->m_linkVid && childVid->m_aniChildVid[8]
			&& childVid->m_weapon) {
...
void SPRITE::InsertItem(int p_vidIdx)
{
	...
	if (n >= max) {
		...
		if (newMax > max) {
			...
			list->m_data = (int*) operator new(4 * newMax);
			if (!list->m_data)
