...
int R_CODER::start_encoding(char p_byte, int p_cache, FILE* p_file)
{
	int result = 0;
	if (!m_file) {
		m_file = p_file;
...
int R_CODER::EndEncoding()
{
	...
	fputc(out, m_file);
	fputc((m_cache >> 16) & 0xFF, m_file);
	fputc((m_cache >> 8) & 0xFF, m_file);
	fputc(m_cache & 0xFF, m_file);
	return m_cache;
}
