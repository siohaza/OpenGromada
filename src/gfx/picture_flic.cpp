...
DECOMP_SIZE_ASSERT(FLIC_HEADER, 0x80)
...
int PICTURE_FLIC::NextFrame()
{
	...
	if (*(unsigned short*) frame != 0xf1fa) {
		operator delete(frame);
		return (int) MYERROR::Error(::Error,
			"PICTURE '%s'", 4,
			// STRING: ALIEN 0x4837cc
			"chunk mark", m_frame, m_name);
	}

	unsigned int noChunks = *(unsigned short*) (frame + 2);
	unsigned int chunkOffset = 12;
	for (unsigned int chunkNo = 0; chunkNo < noChunks; ++chunkNo) {
		unsigned int chunkSize = *(unsigned int*) (frame + chunkOffset);
		...
		case 7: { // FLI_SS2
			int noLines = *(unsigned short*) data;
			data += 2;
			int dstOffset = 0;
			int line = 0;
			while (line < noLines) {
				...
				if (!(control & 0xc000)) {
					for (int packet = 0; packet < control; ++packet) {
						...
						if (words < 0) {
							int word;
							while (++words <= 0) {
								*(unsigned short*) (m_pixels + dstOffset) =
									*(unsigned short*) data;
								dstOffset += 2;
							}
							data += 2;
