...
int PICTURE_BMP::LoadHeader(BMP_HEADER* p_header)
{
	...
	if (p_header->m_compression) {
		...
		MYERROR::Error(::Error,
			...
			"not supported bmp compression type", 0, m_name);
		return 1;
	}
	if (p_header->m_depth < 8) {
		...
		MYERROR::Error(::Error,
			...
			"not supported bmp 2 and 4 bit type", 0, m_name);
		return 1;
	}
	...
	MYERROR::Error(::Error,
		...
		"BMP parameters different from first cadr", m_frame, m_name);
	return 1;
}
...
int PICTURE_BMP::Rewind()
{
	...
	if (result) {
		if (m_file)
			fclose(m_file);
		m_file = *m_name.m_str ? fopen(m_name.m_str, "rb") : 0;
		BMP_HEADER header;
...
int PICTURE_BMP::NextFrame()
{
	...
	{
		char* temporary;
		char* numberedName = *((STRING*) &m_name)->Add(&temporary, m_frame);
