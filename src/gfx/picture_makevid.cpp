...
int VID_TEXCOOR::Intersection(int p_x, int p_y, int p_w, int p_h) const
{
...
inline static PICTURE_BASE* ImplOf(const PICTURE& p_picture)
{
	return p_picture.m_impl;
}

inline static void SetColorFromPixel(COLOR* p_color, const int* p_pixel)
{
	p_color->m_value = *p_pixel;
}

inline static int ColorDiff(const COLOR* p_color, const COLOR& p_other)
{
...
inline static COLOR SetColorAlpha(COLOR* p_color, int p_alpha)
{
	if (p_alpha < 0) {
		p_alpha = 0;
	}
...
int PICTURE_MAKEVID::Load(STRING p_name, STRING p_alpha, STRING p_z)
{
	...
	const char* colorName = p_name.m_str;
	const char* alphaName = p_alpha.m_str;
	const char* zName = p_z.m_str;
	int anyOk = m_color.Load(&colorName);
	anyOk = anyOk == 0;
	anyOk |= m_alpha.Load(&alphaName) == 0;
	m_z.Load(&zName);
...
void PICTURE_MAKEVID::SetPaletteDecodeNumber(COLOR p_color, unsigned char p_number)
{
	...
	if (table) {
		unsigned int c = p_color.m_value;
		unsigned char* slot =
			table + 16 * (((c & 0xf8) | (((c & 0xfc00) | ((c >> 3) & 0x1f0000)) >> 2)) >> 3) + (c >> 28);
		int old = *slot;
		if (!old || old == p_number) {
			*slot = p_number;
		}
...
unsigned int PICTURE_MAKEVID::GetPixelT(int p_x, int p_y)
{
...
int PICTURE_MAKEVID::WriteSurfaces(unsigned char* p_out, char* p_src, int p_width, int p_height)
{
	int pos = 0;
	...
	// No shipped asset requests the unsupported DXT conversion path.
	if (::Error) {
		MYERROR::Error(::Error, "PICTURE", 4, "DXT compression is not supported", 0);
	}
	return 0;
}
...
void PICTURE_MAKEVID::WriteHardware(RESOURCE* p_res)
{
	...
	int cells =
		m_color.m_impl->m_noFrames * (m_color.m_impl->m_width / 256 + 1) * (m_color.m_impl->m_height / 256 + 1) + 1;
	...
	VID_CHILD** sorted = (VID_CHILD**) operator new(sizeof(*sorted) * count);
	if (!sorted) {
		{
			STRING name(m_color.m_impl->m_name);
			MYERROR::Error(
	...
	qsort(sorted, count, sizeof(*sorted), SortCallBack);
	...
	if (!zPages) {
		{
			STRING name(m_color.m_impl->m_name.m_str);
			MYERROR::Error(
				::Error,
				"PICTURE '%s'",
				2,
		...
		if (coords[frame].m_texture >= 0) {
			...
			while (cell) {
				for (int x = cell->m_offsetX; x < cell->m_offsetX + cell->m_w; ++x) {
					for (int y = cell->m_offsetY; y < cell->m_offsetY + cell->m_h; ++y) {
						int at = cell->m_x + x - cell->m_offsetX + ((y + cell->m_y - cell->m_offsetY) << 8);
						if (m_unk0x42c & 4) {
	...
	short pageWord = (short) ((m_unk0x42c & 4) ? 2 * pages : pages);
	...
	for (int page = 0; page < pages; ++page) {
		...
		word = (short) surfH;
		p_res->Write(&word, 2);
		int size = WriteSurfaces((unsigned char*) stream, (char*) colorPages + 0x20000 * page, surfW, surfH);
		p_res->Write(&size, 4);
		p_res->WritePacked(stream, size, colorCoder);
		if (m_unk0x42c & 4) {
			size = 0;
			for (int row = 0; row < surfH; ++row) {
				memcpy(stream + size, (char*) zPages + 512 * (row + (page << 8)), 2 * surfW);
				size += 2 * surfW;
			}
			p_res->Write(&size, 4);
...
inline static void SetShadowWords(int* p_words, int p_value)
{
	*p_words = p_value;
}

inline static int ShadowDimension(int p_value)
{
	return p_value;
}

inline static int ShadowLengthSquared(int p_x, int p_y)
{
...
int PICTURE_MAKEVID::GetShadow(char* p_out)
{
	...
	PackedWrite<short>(p_out, 0);
	if (!(m_unk0x42c & 0x20000)) {
		return 2;
	}

	short* points =
		(short*) operator new(4 * ShadowDimension(m_color.m_impl->m_height) * ShadowDimension(m_color.m_impl->m_width));
	if (!points) {
		STRING name(m_color.m_impl->m_name);
		MYERROR::Error(
			::Error,
	...
	unsigned char* visited = (unsigned char*) operator new(
		ShadowDimension(m_color.m_impl->m_height) * ShadowDimension(m_color.m_impl->m_width)
	);
	if (!visited) {
		STRING name(m_color.m_impl->m_name);
		MYERROR::Error(
	...
	for (;;) {
		...
		while (base < count) {
			...
			for (probe = base + 3; probe < count; ++probe) {
				if (points[2 * probe + 1] - points[2 * base + 1] == 0 && points[2 * probe] - points[2 * base] == 0) {
					continue;
				}
				...
				for (mid = base + 1; mid < probe; ++mid) {
					int len = Sqrt(ShadowLengthSquared(
						points[2 * base] - points[2 * probe],
					...
					if (abs((points[2 * probe + 1] - points[2 * base + 1]) * points[2 * mid] +
							...
							points[2 * base] * points[2 * probe + 1] + points[2 * probe] * points[2 * base + 1]) /
							len >
			...
			base = probe - 1;
			if (probe < count) {
				dst[0] = points[2 * probe - 2];
				dst[1] = points[2 * probe - 1];
				++keptCount;
				dst += 2;
			}
		}
		...
		while (keptCount > 255) {
			...
			for (int k = 0; k < keptCount; ++k) {
				points[2 * k] = points[4 * k];
				points[2 * k + 1] = points[4 * k + 1];
			}
		}
		PackedWrite<short>(p_out, (short) keptCount);
		for (int k = 0; k < keptCount; ++k) {
			int z = GetPixelZ(points[2 * k], points[2 * k + 1]);
			int lift = z / 8 - 128;
			PackedWrite<short>(p_out + 2 * words, points[2 * k]);
			++words;
			PackedWrite<short>(p_out + 2 * words, (short) (points[2 * k + 1] + lift));
			++words;
			PackedWrite<short>(p_out + 2 * words, (short) lift);
			++words;
		}
		operator delete(points);
		operator delete(visited);
		...
		for (int y = 0; y < m_color.m_impl->m_height; ++y) {
			for (int x = 0; x < m_color.m_impl->m_width; ++x) {
				...
				if (matrix[pairJ][pairI] < distRow[bestIdx]) {
					SetPaletteDecodeNumber(COLOR((int) m_palette[pairI]), (unsigned char) bestIdx);
...
void PICTURE_MAKEVID::WriteSoftware(RESOURCE* p_res)
{
	...
	for (int frame = 0; frame < m_color.m_impl->m_noFrames; ++frame) {
		...
		int crc = CalcCRC32();
		crcs[frame] = crc;
		// Retain the search index for the post-loop completion test.
		int dup;
		for (dup = 0; dup < frame; ++dup) {
			if (crc == crcs[dup]) {
				pos = 2;
				PackedWrite<short>(buf, (short) dup);
				break;
			}
		}
		if (dup >= frame) {
			...
			PackedWrite<short>(buf + pos, (short) top);
			pos += 2;
			PackedWrite<short>(buf + pos, (short) (bottom - top));
			pos += 2;
				...
				for (;;) {
					...
					if (m_unk0x42c & 4) {
						for (int zx = runStart; zx < x; ++zx) {
							PackedWrite<short>(buf + pos, GetPixelZ(zx, y));
							pos += 2;
						}
					}
					for (int px = runStart; px < x; ++px) {
						if (m_unk0x42c & 8) {
							...
							PackedWrite<short>(buf + pos, (short) GetPixelT(px, y));
							pos += 2;
						}
					}
				}
...
void PICTURE_MAKEVID::WritePseudo3d(RESOURCE* p_res)
{
	char* buf = (char*) operator new(0x200000);
	if (!buf) {
		MYERROR::Error(::Error, "PICTURE '%s'", 2, "cdr_s", 0, STRING(m_color.m_impl->m_name).m_str);
		exit(1);
	}
	int* crcs = (int*) operator new(4 * GetNoFrames());
	if (!crcs) {
		MYERROR::Error(::Error, "PICTURE '%s'", 2, "cntrl_s", 0, STRING(m_color.m_impl->m_name).m_str);
		exit(1);
	}
	QS1_CODER* coder;
	if (m_unk0x42c & 0x100) {
	...
	for (int frame = 0; frame < m_color.m_impl->m_noFrames; ++frame) {
		...
		if (dup >= frame) {
			...
			GetRectangle(&left, &top, &right, &bottom);
			if (m_unk0x42c & 8) {
				PackedWrite<int>(buf + pos, 41);
			}
			else if (m_unk0x42c & 2) {
				PackedWrite<int>(buf + pos, 21);
			}
			else {
				PackedWrite<int>(buf + pos, 25);
			}
			pos += 4;
			PackedWrite<short>(buf + pos, (short) (right - left));
			pos += 2;
			PackedWrite<short>(buf + pos, (short) (bottom - top));
			pos += 2;
			int y;
			for (y = top; y < bottom; ++y) {
				for (int x = left; x < right; ++x) {
					if (m_unk0x42c & 8) {
						...
						PackedWrite<unsigned int>(buf + pos, GetPixelT(x, y));
						pos += 4;
					}
					else {
						PackedWrite<short>(buf + pos, (short) GetPixelT(x, y));
						pos += 2;
					}
				}
			}
			...
			for (y = top; y < bottom + 8; y += 8) {
				for (int x = left; x < right + 8; x += 8) {
					if (IsPixelInBox(x - 8, y - 8, 9, 9)) {
						...
						PackedWrite<short>(buf + pos, (short) x);
						pos += 2;
						PackedWrite<short>(buf + pos, (short) y);
						pos += 2;
						PackedWrite<short>(buf + pos, z);
						pos += 2;
						PackedWrite<short>(buf + pos, (short) (x - left));
						pos += 2;
						PackedWrite<short>(buf + pos, (short) (y - top));
						pos += 2;
			...
			PackedWrite<int>(buf + countPos, vertices);
			for (y = top; y < bottom + 8; y += 8) {
				for (int x = left; x < right + 8; x += 8) {
					if (IsPixelInBox(x, y, 8, 8)) {
						PackedWrite<short>(buf + pos, grid[x - left + gridW * (y - top)]);
						pos += 2;
						PackedWrite<short>(buf + pos, grid[x + 8 - left + gridW * (y - top)]);
						pos += 2;
						PackedWrite<short>(buf + pos, grid[x - left + gridW * (y - top + 8)]);
						pos += 2;
						PackedWrite<short>(buf + pos, grid[x + 8 - left + gridW * (y - top + 8)]);
						pos += 2;
						PackedWrite<short>(buf + pos, grid[x + 8 - left + gridW * (y - top)]);
						pos += 2;
						PackedWrite<short>(buf + pos, grid[x - left + gridW * (y - top + 8)]);
			...
			PackedWrite<int>(buf + countPos + 4, indices);
